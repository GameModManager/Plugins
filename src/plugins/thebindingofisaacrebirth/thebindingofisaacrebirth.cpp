 ...
 *                        overrides(35), graphics(40), music(45), late(50),
 * unknown(99)
...
static const int IsaacCategoryIds[] = {
    CAT_ITEMS,   CAT_ACTIVE_ITEMS, CAT_TRINKETS,  CAT_PILLS,
    CAT_CARDS,   CAT_PICKUPS,      CAT_LUA,       CAT_ROOMS,
    CAT_FLOORS,  CAT_PLAYER_CHARS, CAT_FAMILIARS, CAT_BABIES,
    CAT_ENEMIES, CAT_GRAPHICS,     CAT_SHADERS,   CAT_SOUND_EFFECTS,
    CAT_MUSIC,   CAT_BOSSES,       CAT_HAZARDS,   CAT_CHALLENGES,
...
static const char *const IsaacCategoryNames[] = {
    ...
    "Cards",     "Pickups",
    "Lua",       "Rooms",
    "Floors",    "Player Characters",
    "Familiars", "Babies",
    "Enemies",   "Graphics",
    "Shaders",   "Sound Effects",
    "Music",     "Bosses",
    "Hazards",   "Challenges",
    "Tweaks",    "Removals",
};
...
void gmm_register_v1(GmmRegistrationCtx *ctx) {
  ...
  ctx->register_identity(ctx, STEAM_APPID, NULL, /* gog_id */
                         NULL,                   /* epic_namespace */
                         NEXUS_DOMAIN, "The Binding of Isaac: Rebirth",
  ...
  if (ctx->register_meta) {
    ctx->register_meta(ctx, "GameModManager Team", "1.0.0",
  ...
  if (ctx->register_categories) {
    ctx->register_categories(ctx, IsaacCategoryIds, IsaacCategoryNames,
                             IsaacCategoryParentIds, IsaacCategoryCount);
  }
  ...
  ctx->register_tab(
      ctx, "mods", "Mods", "mods/",
      "Isaac mod folders (rename to reorder, disable.it to toggle)", NULL, NULL,
      NULL, NULL, NULL);

  ctx->register_tab(ctx, "downloads", "Downloads", "mods/",
                    "Download mods from Steam Workshop or Nexus Mods", "nxm",
                    "nexusmods.com", "nexus", NULL, NULL);

  ctx->register_tab(ctx, "conflicts", "Conflicts", "mods/",
                    "File conflicts between installed mods", NULL, NULL, NULL,
                    NULL, "data");

  /* -- Game-dependent hooks -- */

  /* Conflict extensions -- Isaac's asset/script formats */
  ctx->register_hook(ctx, "conflict_extensions", CONFLICT_EXTENSIONS, NULL, 0,
                     NULL);

  /* Ignored files -- skip during conflict scanning */
  ctx->register_hook(ctx, "ignored_files", IGNORED_FILES, NULL, 0, NULL);

  /* Disable mechanism -- Isaac checks for this file to skip loading */
  ctx->register_hook(ctx, "disable_mechanism", DISABLE_MECHANISM, NULL, 0,
                     NULL);
  ...
  ctx->register_hook(ctx, "delayed_disable", "true", NULL, 0, NULL);

  /* Metadata file -- which file to read for mod name/version */
  ctx->register_hook(ctx, "metadata_file", METADATA_FILE, NULL, 0, NULL);

  /* Metadata name tag -- XML tag containing the mod display name */
  ctx->register_hook(ctx, "metadata_name_tag", METADATA_NAME_TAG, NULL, 0,
                     NULL);

  /* Metadata version tag -- XML tag containing the mod version */
  ctx->register_hook(ctx, "metadata_version_tag", METADATA_VERSION_TAG, NULL, 0,
                     NULL);

  /* Priority prefix regex -- strip this from display name */
  ctx->register_hook(ctx, "priority_prefix_re", PRIORITY_PREFIX_RE, NULL, 0,
                     NULL);

  /* Priority format -- printf format for the NNN prefix */
  ctx->register_hook(ctx, "priority_format", PRIORITY_FORMAT, NULL, 0, NULL);

  /* Workshop ID pattern -- extract from folder name suffix _<digits> */
  ctx->register_hook(ctx, "workshop_id_pattern", WORKSHOP_ID_PATTERN, NULL, 0,
                     NULL);

  /* Auto-sort group definitions -- JSON array of {name, priority} */
  ctx->register_hook(ctx, "auto_sort_groups", AUTO_SORT_GROUPS, NULL, 0, NULL);
  ...
  ctx->register_hook(ctx, "workshop_tag_categories", WORKSHOP_TAG_CATEGORIES,
                     NULL, 0, NULL);
  ...
  ctx->register_hook(ctx, "steam_app_name", "The Binding of Isaac Rebirth",
                     NULL, 0, NULL);

  /* Mods subfolder -- relative to game install root */
  ctx->register_hook(ctx, "mods_subpath", "mods", NULL, 0, NULL);

  /* Mod scan subpath -- scan game_dir/mods (same as mods_subpath for Isaac).
   * Explicitly declared so the scanner uses the correct scan path when
   * mod_scan_subpath is checked before mods_subpath. */
  ctx->register_hook(ctx, "mod_scan_subpath", "mods", NULL, 0, NULL);
  ...
  ctx->register_hook(ctx, "deploy_prefix", "mods", NULL, 0, NULL);
  ...
  ctx->register_hook(
      ctx, "game_mods_dir",
      "~/Library/Application Support/Binding of Isaac Afterbirth+ Mods", NULL,
      0, NULL);
#endif
  ...
  ctx->register_hook(ctx, "deploy_include_mod_id", "true", NULL, 0, NULL);

  /* Game versions -- Isaac version string -> release date */
  ctx->register_hook(ctx, "game_versions", GAME_VERSIONS, NULL, 0, NULL);
  ...
  ctx->register_hook(ctx, "steam_appid", "250900", NULL, 0, NULL);
  ...
  ctx->register_hook(ctx, "download_sources", "Nexus,Steam", NULL, 0, NULL);

  /* Mod counter label -- how to label the mod count in status bar.
   * "Mods" = raw mod folders, "Plugins" = ESM/ESP-style plugins. */
  ctx->register_hook(ctx, "mod_counter_label", "Mods", NULL, 0, NULL);
  ...
  ctx->register_hook(ctx, "conflict_order_reversed", "true", NULL, 0, NULL);

  /* Conflict scan dirs -- Isaac mods only conflict in these subdirectories;
   * anything outside is not read by the game engine. */
  ctx->register_hook(ctx, "conflict_scan_dirs", "resources,resources-dlc3",
                     NULL, 0, NULL);
  ...
  ctx->register_hook(ctx, "mod_valid_dirs", "resources,resources-dlc3", NULL, 0,
                     NULL);
  ...
  ctx->register_hook(ctx, "uses_merged", "true", NULL, 0, NULL);
}
