 ...
 * "File Support" plugin, not a game) and provides an animation parser for
 * .anm2 files (Isaac animation format).
 *
...
void gmm_register_v2(GmmRegistrationCtxV2* ctx) {
   ...
   * register_game would set game_support = true and make it appear in the
   * game selector, which is wrong for a file-format plugin. It is discovered
   * and listed via register_plugin + register_category instead. */
