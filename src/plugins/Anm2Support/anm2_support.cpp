...
static int32_t to_int32(const std::string& s, int32_t fallback = 0) {
    try { return static_cast<int32_t>(std::stoi(s)); } catch (...) { return fallback; }
}
...
static int parse_anm2(const char* file_path, const char* base_dir,
    ...
    if (!find_line(content, "<Animation", anim_line)) return 0;
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (layer_pos == std::string::npos) break;

            /* Extract layer line */
            size_t l_line_start = frame_content.rfind('\n', layer_pos);
            if (l_line_start == std::string::npos) l_line_start = 0;
            else l_line_start++;
            size_t l_line_end = frame_content.find('\n', layer_pos);
            if (l_line_end == std::string::npos) l_line_end = frame_content.length();
            std::string layer_line = frame_content.substr(l_line_start, l_line_end - l_line_start);
            ...
            if (layer_scope_end == std::string::npos) layer_scope_end = frame_content.length();
            std::string layer_scope = frame_content.substr(layer_scope_start, layer_scope_end - layer_scope_start);
...
void gmm_register_v1(GmmRegistrationCtx* ctx) {
    ...
    if (ctx->register_animation_parser) {
        ctx->register_animation_parser(ctx,
            nullptr,            /* game_id — NULL = non-game-specific */
            "anm2",             /* file extension */
