...
void IsaacAutoSorter::parse_masterlist_yaml(const YAML::Node &root,
  ...
  if (root["groups"] && root["groups"].IsSequence()) {
    for (const auto &g : root["groups"]) {
      SortGroup sg;
      if (g["name"])
        sg.name = g["name"].as<std::string>();
      if (g["priority"])
        sg.priority = g["priority"].as<int>();
      groups.push_back(sg);
    }
  }
  ...
  if (root["mods"] && root["mods"].IsSequence()) {
    for (const auto &m : root["mods"]) {
      IsaacModEntry me;
      if (m["id"])
        me.workshop_id = m["id"].as<int64_t>();
      if (m["name"])
        me.name = m["name"].as<std::string>();
      if (m["group"])
        me.group = m["group"].as<std::string>();
      if (m["after"])
        me.after = yaml_to_int_list(m["after"]);
      if (m["before"])
        me.before = yaml_to_int_list(m["before"]);
      if (m["requires"])
        me.required_by = yaml_to_int_list(m["requires"]);
      if (m["pattern"])
        me.pattern = m["pattern"].as<std::string>();
      if (m["tag"])
        me.tag = m["tag"].as<std::string>();
      if (m["preserve_name"])
        me.preserve_name = m["preserve_name"].as<bool>();
      ...
      if (m["tags"] && m["tags"].IsSequence()) {
        for (const auto &t : m["tags"]) {
          MasterlistTag mt;
          if (t["type"])
            mt.type = t["type"].as<std::string>();
          if (t["message"])
            mt.message = t["message"].as<std::string>();
          if (t["condition"]) {
            const auto &cond = t["condition"];
            if (cond["if_installed"])
              mt.if_installed = yaml_to_int_list(cond["if_installed"]);
            if (cond["if_not_installed"])
              mt.if_not_installed = yaml_to_int_list(cond["if_not_installed"]);
          }
...
bool IsaacAutoSorter::should_preserve_name(int64_t workshop_id) const {
  for (const auto &e : mod_entries_)
    if (e.workshop_id == workshop_id && e.preserve_name)
      return true;
...
std::vector<MasterlistTag> IsaacAutoSorter::evaluate_tags(
    int64_t workshop_id, const std::vector<int64_t> &installed_workshop_ids) const {
  // Find the mod entry
  ...
  std::unordered_set<int64_t> installed_set(installed_workshop_ids.begin(),
                                            installed_workshop_ids.end());
...
const IsaacModEntry *IsaacAutoSorter::match_mod(const IsaacSortModInfo &mod) const {
  ...
  if (mod.workshop_id != 0) {
    for (const auto &e : user_entries_)
      if (e.workshop_id == mod.workshop_id)
        return &e;
    for (const auto &e : mod_entries_)
      if (e.workshop_id == mod.workshop_id)
        return &e;
...
std::vector<std::string> IsaacAutoSorter::topological_sort(
  ...
  for (const auto &mod : group_mods) {
    SortItem item;
    item.key         = (mod.workshop_id != 0) ? "id:" + std::to_string(mod.workshop_id)
  ...
  for (const auto &mod : group_mods) {
    ...
    for (int64_t dep : entry->after) {
      ...
      if (key_to_idx.count(dk)) {
        graph[dk].push_back(mod_key);
        in_degree[mod_key]++;
      }
    }
  ...
  for (const auto &item : items)
    key_to_folder[item.key] = item.folder_name;

  std::vector<std::string> out;
  for (const auto &k : result) {
    auto it = key_to_folder.find(k);
    if (it != key_to_folder.end())
...
IsaacAutoSorter::auto_sort(const std::vector<IsaacSortModInfo> &mods) const {
